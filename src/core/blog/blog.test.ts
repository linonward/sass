...
describe("sitemap", () => {
  test("博客列表和已发布文章，hreflang 只列出有翻译的语言", () => {
    expect(blogSitemap()).toMatchSnapshot();
  });

  test("合并进 app/sitemap.ts", () => {
    const urls = sitemap().map((entry) => entry.url);
    expect(urls).toContain("https://sass.linonward.com/blog/hello");
    expect(urls).toContain("https://sass.linonward.com/de/blog/hello");
...
describe("RSS", () => {
  ...
  test("默认语言的链接不带前缀，草稿不出现", () => {
    ...
    expect(xml).toContain("<link>https://sass.linonward.com/blog/hello</link>");
